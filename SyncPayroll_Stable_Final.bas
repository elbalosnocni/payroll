...
' ========================================================================
' CO CHE RETRY ON DINH
' ------------------------------------------------------------------------
' QUAN TRONG:
' Khi WinHTTP timeout, request POST KHONG dong nghia GAS da dung.
' GAS co the van dang ghi Sheet va giu ScriptLock. Neu POST lai ngay sau
' timeout, request moi se nhan SYNC_BUSY. Vi vay timeout duoc xem la
' "request co kha nang dang chay", khong phai loi de retry ngay.
' ========================================================================
Private Const CLIENT_MAX_ATTEMPTS As Long = 3
Private Const CLIENT_TIMEOUT_RECOVERY_WAIT_SEC As Long = 90
Private Const CLIENT_BUSY_WAIT_SEC As Long = 60
Private Const CLIENT_TRANSIENT_WAIT_SEC As Long = 45
...
Private Sub SyncOneWorkshop(ByVal rootPath As String, ByVal filePrefix As String, _
    ...
    LogMessage "[" & label & "] Doc duoc " & payroll.Count & _
               " dong luong, " & employees.Count & " nhan vien (da bo qua TOTAL/SUBTOTAL/header)."
...
Private Sub ReadSalarySheet(ByVal ws As Worksheet, ByVal dscnvLookup As Object, _
    ...
    ' KHONG cat bang tai TOTAL cuoi cung.
    ' Sheet Salary co the co nhieu TOTAL/SUBTOTAL/header, ke ca 2 dong TOTAL lien nhau.
    ' Doc toan bo vung co du lieu va CHI nhan nhung dong thuc su la nhan vien.
    Dim seenManv As Object
    Set seenManv = CreateObject("Scripting.Dictionary")

    Dim r As Long
    For r = DATA_START_ROW To rawLastRow
        Dim maNV As String
        Dim hoTen As String
        ...
        ' Bo qua moi TOTAL/SUBTOTAL/header va dong khong phai nhan vien.
        If Not IsSalaryEmployeeRow(ws, r, maNV, hoTen) Then GoTo ContinueLoop
...
Private Function IsSalaryEmployeeRow(ByVal ws As Worksheet, ByVal row As Long, _
                                       ByVal maNV As String, ByVal hoTen As String) As Boolean
    Dim cText As String
    cText = UCase$(Trim$(SafeText(GetCellText(ws, row, COL_SALARY_TOTAL_MARKER))))

    ' 1) TOTAL/SUBTOTAL/GROUP TOTAL: luon bo qua, ke ca nhieu dong va lien tiep.
    If ContainsTotalMarker(cText) Then
        LogMessage "  [Salary] Bo qua dong TOTAL/SUBTOTAL tai dong " & row & _
                   " | C=" & cText
        IsSalaryEmployeeRow = False
        Exit Function
    End If

    ' 2) Dong trong / dong ngat / dong khong co MaNV + HoTen.
    If maNV = "" Or hoTen = "" Then
        IsSalaryEmployeeRow = False
        Exit Function
    End If

    ' 3) Header thuong lap lai o giua cac nhom/bang.
    If IsSalaryHeaderText(maNV) Or IsSalaryHeaderText(hoTen) Then
        LogMessage "  [Salary] Bo qua dong header tai dong " & row & _
                   " | D=" & maNV & " | CF=" & hoTen
        IsSalaryEmployeeRow = False
        Exit Function
    End If

    IsSalaryEmployeeRow = True
End Function

Private Function ContainsTotalMarker(ByVal s As String) As Boolean
    s = UCase$(Trim$(s))
    If s = "" Then Exit Function

    ' Bat duoc TOTAL, SUBTOTAL, GRAND TOTAL, TOTAL GROUP...,
    ' khong phu thuoc viet hoa/viet thuong.
    ContainsTotalMarker = _
        (InStr(1, s, "SUBTOTAL", vbTextCompare) > 0) Or _
        (InStr(1, s, TOTAL_ROW_KEYWORD, vbTextCompare) > 0) Or _
        (InStr(1, s, "TONG", vbTextCompare) > 0)
End Function

Private Function IsSalaryHeaderText(ByVal s As String) As Boolean
    s = UCase$(Trim$(SafeText(s)))
    Select Case s
        Case "MÃ NV", "MA NV", "MANV", "EMPLOYEE ID", "EMPLOYEE CODE", _
             "CODE", "ID", "MÃ NHÂN VIÊN", "MA NHAN VIEN", _
             "HỌ TÊN", "HO TEN", "NAME", "TÊN", "TEN"
            IsSalaryHeaderText = True
        Case Else
            IsSalaryHeaderText = False
    End Select
End Function
...
Private Function PostJsonUtf8WithRetry(ByVal url As String, ByVal jsonBody As String, _
    ...
    For attempt = 1 To CLIENT_MAX_ATTEMPTS

        LogMessage "[" & label & "] HTTP POST lan " & attempt & "/" & _
                   CLIENT_MAX_ATTEMPTS & "..."

        responseText = PostJsonUtf8(url, jsonBody)

        timedOut = IsTimeoutResponse(responseText)

        If timedOut Then
            ' KHONG retry ngay.
            ' Request truoc co the van dang chay tren GAS va dang giu ScriptLock.
            If attempt < CLIENT_MAX_ATTEMPTS Then
                waitSec = CLIENT_TIMEOUT_RECOVERY_WAIT_SEC
                LogMessage "[" & label & "] HTTP TIMEOUT: request co the VAN DANG CHAY tren GAS." & _
                           " Khong POST lai ngay; cho " & waitSec & _
                           "s de GAS co thoi gian ket thuc, sau do moi kiem tra lai."
                SleepSeconds waitSec
            Else
                Exit For
            End If

        ElseIf ResponseIsOk(responseText) Then
        ...
        Else
            ...
            If attempt < CLIENT_MAX_ATTEMPTS Then
                ...
                Select Case UCase$(code)
                    Case "SYNC_BUSY"
                        ' Request truoc dang giu lock. Cho lau hon lock wait
                        ' cua GAS (25s) de tranh vong lap BUSY nhanh.
                        waitSec = CLIENT_BUSY_WAIT_SEC
                        LogMessage "[" & label & "] GAS tra SYNC_BUSY." & _
                                   " Cho " & waitSec & "s roi moi gui lai."

                    Case "SYNC_ERROR", "INTERNAL_ERROR"
                        waitSec = CLIENT_TRANSIENT_WAIT_SEC
                        LogMessage "[" & label & "] GAS tra " & code & "." & _
                                   " Cho " & waitSec & "s roi moi gui lai."

                    Case Else
                        waitSec = CLIENT_TRANSIENT_WAIT_SEC
                        LogMessage "[" & label & "] Loi tam thoi (" & code & ")." & _
                                   " Cho " & waitSec & "s roi moi gui lai."
                End Select
    ...
    If timedOut Then
        Err.Raise vbObjectError + 2202, "PostJsonUtf8WithRetry", _
                  "[" & label & "] HTTP timeout sau " & CLIENT_MAX_ATTEMPTS & _
                  " lan. Khong tiep tuc POST de tranh tao nhieu phien dong bo chong len nhau." & _
                  vbCrLf & "Hay kiem tra SyncStatus/Execution cua GAS; request dau tien co the da hoan tat sau khi VBA timeout."
    Else
        Err.Raise vbObjectError + 2203, "PostJsonUtf8WithRetry", _
                  "[" & label & "] Khong dong bo duoc sau " & CLIENT_MAX_ATTEMPTS & _
                  " lan. Phan hoi cuoi: " & responseText
    End If
End Function

Private Function IsTimeoutResponse(ByVal responseText As String) As Boolean
    Dim s As String
    s = LCase$(responseText)

    ' WinHTTP 0x80072EE2 = -2147012894: ERROR_WINHTTP_TIMEOUT.
    IsTimeoutResponse = _
        (InStr(1, s, "-2147012894", vbTextCompare) > 0) Or _
...
Private Function IsRetryableResponse(ByVal responseText As String, ByVal code As String) As Boolean
    If IsTimeoutResponse(responseText) Then
