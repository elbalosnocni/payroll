...
var CONFIG = {
  ...
  // Thời gian tối đa (ms) chờ để lấy được khóa đồng bộ (LockService) trước khi
  // trả lỗi SYNC_BUSY. Nhờ upsertEmployees/upsertPayroll đã được tối ưu ghi
  // theo lô (batch), 1 lần đồng bộ thực tế chỉ mất vài giây nên thời gian chờ
  // này KHÔNG cần dài; để dài vừa phải (25s) chỉ để tránh báo bận nhầm khi 2
  // xưởng gửi lên gần như cùng lúc.
  SYNC_LOCK_WAIT_MS: 25000
};
